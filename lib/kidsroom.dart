...
class Kidsroom extends StatefulWidget {
  @override
  _KidsroomState createState() => _KidsroomState();
}
...
class _KidsroomState extends State<Kidsroom> {
  ...
  Future NetworkingLight() async {
    bool auxo = await firebaseController.syncData("KidsRoom", "Light", "Off", "On");
    String auxi = await firebaseController.getData("KidsRoom", "Light");
    setState(() {
      RealValueLight = auxi;
      testAuxoLight(auxo);
      visibility = false;
  ...
  void NetworkingInit() async {
    ...
    String Light = await firebaseController.getData("KidsRoom", "Light");
